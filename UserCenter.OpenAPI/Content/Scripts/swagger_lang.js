            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#resources_container .resource").each(function (i, item) {
                        id = $(item).attr("id");
                        if (id) {
                            controllerName = id.substring(9, id.indexOf("Controller"));
                            strSummary = summaryDict[controllerName];
                            if (strSummary) {
                                var ulObj = $(item).children(".heading").children(".options");
                                if (ulObj.children().length == 3) {
                                    ulObj.prepend('<li class="controller-summary" title="' + strSummary + '">' + strSummary + '</li>');
                                }
                            }
                        }
                    });
